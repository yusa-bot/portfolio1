...
export const ZennToc: React.FC<TocProps> = ({ article }) => {
  const [toc, setToc] = useState<TocItem[]>([]);

  useEffect(() => {
    if (article?.toc && article.toc.length > 0) {
      // 最小レベルを計算
      const levels = article.toc.map(item => item.level);
      const minLevel = Math.min(...levels);

      // ベースレベルを設定
      const tocWithBase = article.toc.map(item => ({
        ...item,
        base: minLevel,
      }));
              ...
              <div className="flex items-center py-1">
                <span
                  className={`mr-2 inline-block rounded-full ${
                    isTopLevel ? 'bg-blue-500' : 'bg-blue-300'
                  }`}
                  style={{ width: 6, height: 6 }}
                />
                <button
                  onClick={(e) => handleTocItemClick(e, article.url, item.id)}
                  className="text-xs text-left text-slate-600 hover:text-blue-600 hover:underline transition-colors duration-200 leading-relaxed"
                >
                  {item.text}
                </button>
              </div>
