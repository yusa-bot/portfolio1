...
import { subscribeToHud, toggleHud, getHudState } from './ObservabilityHUD/store';
import { Activity, Clock, Server, Database, Zap } from 'lucide-react';

export default function ObservabilityHUD() {
  const [hudState, setHudState] = useState(getHudState());

  useEffect(() => {
    const unsubscribe = subscribeToHud(setHudState);
    return unsubscribe;
  }, []);
          ...
          >
            ...
            <div className="mb-4 p-3 bg-slate-800/50 rounded">
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="font-mono">{hudState.requests.length}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-slate-300">Avg Response Time</span>
                <span className="font-mono">
                  {hudState.requests.length > 0 
                    ? formatDuration(hudState.totalTime / hudState.requests.length)
                    : '0ms'
                  }
                </span>
              </div>
            ...
            <div className="space-y-2">
              ...
              {hudState.requests.slice(-5).reverse().map((request, index) => (
